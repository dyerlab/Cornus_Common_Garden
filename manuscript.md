...
As a first, more limited check on that half-sib assumption -- short of the full pollen-pool and outcrossing-rate analysis called for above -- we asked how heritability estimates would change if the assumption were relaxed to reflect realized rather than assumed sibship structure. Canopy openness is known to influence correlated paternity in this species (Gardiakos 2009), so maternal arrays under the more open canopies typical of cultivar landscapes might plausibly carry a different mix of half- and full-sib offspring than arrays under closed native-forest canopy. We estimated pairwise marker-based relatedness among each mother's own genotyped offspring and pooled these pairwise estimates within each origin class to characterize the distribution of realized sibship relatedness for native and cultivar families separately (Fig_OffspringRelatedness, Supplementary Materials). Re-expressing heritability at each origin's median realized relatedness, in place of the assumed half-sib value of 0.25, shifted point estimates modestly downward for every trait with detectable family variance; traits whose family-variance estimate was already indistinguishable from zero were unaffected, since heritability remains zero regardless of the assumed relatedness. The qualitative pattern reported above held across the full half-sib-to-full-sib range: heritability stayed low in the native subset and moderate in the cultivar subset regardless of which relatedness assumption was used (Fig_Heritability), with the same range reported per trait in Tab_HeritabilityRelatedness (Supplementary Materials).
...
**Fig_Heritability.** Heritability re-expressed under realized within-mother offspring relatedness, for native and cultivar family arrays. The point estimate (dot) is heritability at each origin's median pairwise sibling relatedness; the vertical line spans the range implied by a strict half-sib ($r = 0.25$) to strict full-sib ($r = 0.5$) assumption.
