...
Multilocus genotypes of maternal trees and offspring arrays were used to develop a secondary classification system based on shared ancestry with cultivar varieties, following the approach of Wadl *et al.* (2008). Each seedling was compared with every cultivar voucher genotype and scored at each locus and classified as the proportion of alleles shared with known cultivar varities (Admixture Percentage; AP). Because we did not have sufficient sample sizes to distinguish among individual cultivar varieties, we treated all cultivars identically.  Both maternal sample location (*cultivar* vs. *native*) and admixture percentage were used as covaraites in the following analyses.  
...
Heritability was estimated for each trait from the top-supported model. Survival heritability was estimated on the latent liability scale following Gilmour et al. (1985). For quantitative plant traits, we estimated narrow-sense heritability under a half-sib design, in which maternal family variance is treated as one-quarter of the additive genetic variance. Full model structure, variance decomposition, and heritability derivations, along with model diagnostics and sensitivity analyses, are provided in the Appendix.  All statistical analyses were performed in `R` (version 4.5.3; R Core Team 2026) and all code and data used are available in the online supplementary materials.


# Results

## Sampling Design
To boost the number of native families in this project, we collected seeds from putative native trees at the VCU Rice Rivers Center to augment those families sampled from Pocahontas State Park. In the following analyses, we partitioned the native families by sampling locale to determine if there was a site-effect.  We found no evidence of sample location for putatively native maternal trees for survival or any phenotypic trait (Tab_SiteEffect).  We proceeded by combining samples from the Rice Rivers Center and Pocahontas State Park trees into the single *native* category. 

## Genetic Analysis 

The nine microsatellite markers assayed were highly polymorphic, with 5–26 alleles per locus in maternal individuals and comparable diversity among the offspring genotypes (Tab_MarkerDiversity). Maximum-likelihood estimates of null-allele frequency were low at every locus (all point estimates < 0.07) but their confidence intervals excluded zero at four loci (cf020, cf585, cf597, cf701) in both origin classes (Tab_NullAlleles). We therefore report combined multilocus exclusion probabilities for both the full nine-locus panel and a reduced five-locus panel omitting those four loci. Exclusion probability was high for both the full panel ($P_{excl,multi} > 0.9999$) and the reduced panel ($P_{excl,multi} = 0.999$), indicating that the marker panel retained strong discriminatory power.

## Cultivar Impact Analysis
### Contribution of Origin to Trait Variation

Under the location-based classification, seedling survival to the end of the second growing season was high (native 79.3%, cultivar 83.5%), similar to previous work (Redwine, 2013).  Mortality remained relatively even across the entire experiment (Fig_Survival).  Adding *native*/*cultivar* origin to the generalized linear mixed model did not improve fit over maternal family alone (family: ΔAICc = 0.0, wAICc = 0.54; fixed-effects only: ΔAICc = 1.4, wAICc = 0.27; origin + family: ΔAICc = 2.0, wAICc = 0.19; Tab_Survival), and the origin random effect explained essentially no variance (likelihood-ratio test, χ² ≈ 0, p ≈ 0.5). 

Origin also did not predict the timing of germination itself: adding origin did not improve model fit for germination date over maternal family alone (ΔAICc = 2.0; likelihood-ratio test *p* = 0.48), although germination timing was itself heritable among families (h² = 0.42 [95% CI 0.15, 0.72]; Tab_GerminationTiming).

For plant growth traits, the *family* and *family + origin* models were within ΔAICc ≤ 2 of one another for plant height (PH), stem diameter (SD), number of leaves (NL), above-ground biomass (AGB), below-ground biomass (BGB), and stem biomass (SB), indicating that origin added little beyond family alone (Tab_GrowthAIC); the bootstrap confidence interval for the variance attributed to origin included zero for every trait (Tab_GrowthHeritability). Leaf biomass (LB) was the exception: the origin-inclusive model was clearly favored (ΔAICc = 4.9, wAICc = 0.92 vs. 0.08; Tab_GrowthAIC), although its origin variance component's confidence interval also included zero (Tab_GrowthHeritability). Repeated-measures models showed the same pattern (Tab_RepeatedAIC and Tab_RepeatedHeritability). Together, these results indicate that origin contributed little to trait variation under the location-based classification, with the partial exception of leaf biomass, and that considerable uncertainty remains about the magnitude of any effect.

Under the admixture-based classification, adding Admixture Percentage as a fixed covariate did not improve model fit over maternal family alone for any trait (family alone favored or tied in every case; Tab_GrowthAIC), and the AP coefficient was not significant for any trait (smallest *p* = 0.11, for plant height) or for survival (*p* = 0.88). We therefore found no evidence that cultivar ancestry, whether inferred from maternal location or from genetic admixture, had a meaningful effect on seedling survival, growth, or persistence.

### Family-Level Variation and Trait Heritability

To look at the influence of family on model fit and subsequent quantitative genetic parameter estimation, we split the data into putative native and cultivar sets of offspring arrays based on the location of the maternal tree from which seeds were collected. With this partitioning, the family model was the top-supported model for the cultivar set for every growth trait except leaf biomass, whereas the fixed-effects-only model was best supported for every trait in the native set (Tab_GrowthAIC and Tab_OriginHeritability). For survival, the same split showed that adding family improved model fit in the cultivar subset (ΔAICc = 1.5, wAICc = 0.68 vs. 0.32; likelihood-ratio test χ² = 3.5, df = 1, *p* = 0.030) but not in the native subset, where the fixed-effects-only model remained best supported (ΔAICc = 2.1, wAICc = 0.74 vs. 0.26; Tab_Survival). While there was evidence for family-level variance in survival among cultivar-origin families, the heritability estimate was highly uncertain (h² = 0.51, 95% CI [0, 1.07]). However, family-specific conditional modes (analogous to BLUPs in linear models) extracted from the putative cultivar survival GLMM show variation in survival was heterogeneous among cultivar-origin families (Fig_BLUPs). 

Confidence intervals for the family variance component in the native subset included zero for all plant growth traits, but in the putative cultivar set the variance attributed to family was significant, though modest, for PH (11.9% of phenotypic variance), AGB (8.1%), and SB (11.0%; Tab_OriginHeritability). Heritability estimates were treated as significant when their confidence intervals excluded zero. In the putative cultivar set, heritabilities were significant for the same three traits, ranging from h² = 0.32 [95% CI 0.03, 0.72] for AGB to h² = 0.47 [0.11, 0.93] for PH (Tab_OriginHeritability), consistent with heritable variation among cultivar families and the potential for evolutionary responses to selection when seedlings are grown in understory conditions. 

Sensitivity tests included the removal of influential observations at the family and individual levels. Results for putative cultivar families were robust to the removal of outlier individual observations; if anything, the family effect strengthened (e.g., NL and BGB; Tab_OriginHeritability), and the largest heritability shift was in BGB (h² = 0.23 → 0.36). Removal of the single most influential maternal family had a larger effect on the variance-component and heritability estimates. In most growth traits this family was urb09: for stem biomass, removing urb09 reduced the family variance component ($V_{family}$ = 0.046 → 0.016) and heritability ($h^2$ = 0.51 → 0.19), and weakened but did not eliminate support for the family random effect (likelihood-ratio test χ² = 5.3, df = 1, *p* = 0.022; Tab_GrowthHeritability).   
...
In this study we investigated how the incorporation of cultivar *C. florida* into urban landscaping may influence populations of proximal wild conspecifics *via* cultivar gene escape. To do this we examined early fitness traits of *C. florida* seedlings grown from seeds collected from cultivated and natural landscapes and grown in a common environment. Because we collected open pollinated seedlings that could potentially be native, cultivar, or hybrids, we classified the seedlings based on maternal tree location as well as AP. Broadly, we found that maternal families contributed significantly, though modestly, to variance in plant traits. In contrast, the variance attributed to origin, defined by putative native and cultivar classes, was small and inconsistent across traits. Likewise, when cultivar ancestry was represented by Admixture Percentage, it was not a significant predictor of any plant trait or of survival. Moderate heritability estimates for plant traits among putative cultivar seedlings are consistent with among-family variation upon which selection may act. High overall survival indicates gene flow from cultivated genotypes into native populations may not be self-limiting due to early viability selection. Together, these results indicate that, if established, cultivar-origin offspring are not likely to be disadvantaged compared to native genotypes and therefore have the potential to persist in native forested habitats. This has important implications for the evolutionary trajectory and conservation of conspecifics which inhabit the areas adjacent to human tended cultivars. 

We did not find evidence that seedlings with cultivar provenance will experience different outcomes from native seedlings in terms of survival. However, when the data were analyzed separately for offspring from putative native and cultivar maternal trees, different levels of variation among families became apparent (Fig_BLUPs). While the estimate of narrow-sense heritability of survival among putative cultivar seedlings was imprecise (h² = 0.51 [0, 1.07]), differences among families accounted for roughly 13% of the latent-scale variance in survival. In contrast, no latent-scale survival variance was attributable to family among putative native families. Thus, we found no evidence that early survival differed among broad classifications of origin, but rather family variation among putative cultivar offspring arrays may play an important role. This indicates that which cultivar-origin families escape may be an important factor in cultivar escape and establishment. 
...
| :-- | :-- | --: | --: | --: | --: | --: | --: |
| Maternal | cf020 | 13 | 8.31 | 0.842 | 0.880 | 0.043 | 0.883 |
| Maternal | cf125 | 5 | 3.30 | 0.707 | 0.697 | −0.015 | 0.670 |
| Maternal | cf213 | 26 | 16.29 | 0.948 | 0.939 | −0.010 | 0.942 |
| Maternal | cf273 | 9 | 2.22 | 0.552 | 0.549 | −0.004 | 0.581 |
| Maternal | cf581 | 5 | 1.38 | 0.207 | 0.278 | 0.255 | 0.245 |
| Maternal | cf585 | 18 | 9.74 | 0.862 | 0.897 | 0.039 | 0.893 |
| Maternal | cf597 | 11 | 7.26 | 0.810 | 0.862 | 0.060 | 0.875 |
| Maternal | cf634 | 13 | 6.51 | 0.828 | 0.846 | 0.022 | 0.841 |
| Maternal | cf701 | 9 | 4.90 | 0.690 | 0.796 | 0.134 | 0.792 |
| Maternal | **all loci** | 109 | 59.91 | 0.716 | 0.749 | 0.058 | — |
| Offspring | cf020 | 18 | 8.57 | 0.816 | 0.883 | 0.077 | 0.883 |
| Offspring | cf125 | 6 | 3.01 | 0.670 | 0.668 | −0.004 | 0.670 |
| Offspring | cf213 | 33 | 17.35 | 0.933 | 0.942 | 0.010 | 0.942 |
| Offspring | cf273 | 10 | 2.40 | 0.571 | 0.583 | 0.020 | 0.581 |
| Offspring | cf581 | 7 | 1.32 | 0.234 | 0.243 | 0.036 | 0.245 |
| Offspring | cf585 | 21 | 9.33 | 0.810 | 0.893 | 0.093 | 0.893 |
| Offspring | cf597 | 18 | 8.04 | 0.790 | 0.876 | 0.098 | 0.875 |
| Offspring | cf634 | 17 | 6.25 | 0.830 | 0.840 | 0.012 | 0.841 |
| Offspring | cf701 | 13 | 4.80 | 0.729 | 0.792 | 0.079 | 0.792 |
| Offspring | **all loci** | 143 | 61.07 | 0.709 | 0.747 | 0.047 | — |
...
| cf585 | 0.063 [0.045, 0.083] | 0.017 [0.004, 0.035] | no |
| cf597 | 0.042 [0.025, 0.062] | 0.033 [0.012, 0.059] | no |
| cf634 | 0.010 [0.000, 0.027] | 0.002 [0.000, 0.023] | yes |
| cf701 | 0.022 [0.004, 0.043] | 0.043 [0.021, 0.070] | no |

Combined multilocus exclusion probability (gstudio; Dyer 2009): full nine-locus panel *P*excl = 0.99999; reduced five-locus panel (cf125, cf213, cf273, cf581, cf634) *P*excl = 0.999.
...
| :-- | :-- | --: | --: | --: | :-- | :-- | --: | :-- |
| Total | family | 409.82 | 0.00 | 0.538 | — | 0.289 [0, 0.780] | 0.081 | 0.323 [0, 0.766] |
| Total | fixed only | 411.22 | 1.40 | 0.267 | — | — | — | — |
| Total | origin + family | 411.86 | 2.04 | 0.194 | 0.000 [0, 0] | 0.289 [0, 0.748] | 0.081 | 0.323 [0, 0.741] |
| Native | fixed only | 157.24 | 0.00 | 0.740 | — | — | — | — |
| Native | family | 159.34 | 2.10 | 0.260 | — | 0.000 [0, 0.475] | 0.000 | 0.000 [0, 0.505] |
| Cultivar | family | 252.72 | 0.00 | 0.677 | — | 0.480 [0, 1.205] | 0.127 | 0.510 [0, 1.072] |
| Cultivar | fixed only | 254.20 | 1.48 | 0.323 | — | — | — | — |
| AP | family | 130.45 | 0.00 | 0.734 | — | 0.000 [0, 1.084] | 0.000 | 0.000 [0, 0.992] |
| AP | AP + family | 132.48 | 2.03 | 0.266 | — | — | — | — |

Likelihood-ratio tests: family effect χ² = 3.4, *p* = 0.032 (Total) and χ² = 3.5, *p* = 0.030 (Cultivar); origin effect χ² ≈ 0, *p* ≈ 0.5.
...
| :-- | :-- | --: | --: | --: | --: | --: | --: |
| Total | origin + family | −28.6 | 0.0 | 0.64 | −28.4 | 0.0 | 0.63 |
| Total | family | −27.4 | 1.1 | 0.36 | −27.3 | 1.1 | 0.37 |
| Total | fixed only | 15.9 | 44.5 | 0.00 | 16.0 | 44.4 | 0.00 |
| AP | AP + family | −21.7 | 0.0 | 0.53 | −21.5 | 0.0 | 0.52 |
| AP | family | −21.5 | 0.2 | 0.47 | −21.3 | 0.2 | 0.48 |
| Native | fixed only | 3.4 | 0.0 | 0.69 | 3.7 | 0.0 | 0.71 |
| Native | family | 5.0 | 1.6 | 0.31 | 5.5 | 1.8 | 0.29 |
| Cultivar | family | −40.9 | 0.0 | 1.00 | −40.7 | 0.0 | 1.00 |
| Cultivar | fixed only | −27.0 | 13.9 | 0.00 | −26.9 | 13.8 | 0.00 |

(B) Stem diameter

| Subset | Model | AIC | ΔAIC | *w*AIC | AICc | ΔAICc | *w*AICc |
| :-- | :-- | --: | --: | --: | --: | --: | --: |
| Total | family | −120.8 | 0.0 | 0.70 | −120.6 | 0.0 | 0.70 |
| Total | origin + family | −118.8 | 2.0 | 0.26 | −118.6 | 2.1 | 0.25 |
| Total | fixed only | −115.3 | 5.5 | 0.04 | −115.2 | 5.5 | 0.05 |
| AP | family | −116.6 | 0.0 | 0.70 | −116.4 | 0.0 | 0.71 |
| AP | AP + family | −114.8 | 1.7 | 0.30 | −114.6 | 1.8 | 0.29 |
| Native | fixed only | −10.6 | 0.0 | 0.73 | −10.3 | 0.0 | 0.75 |
| Native | family | −8.6 | 2.0 | 0.27 | −8.1 | 2.2 | 0.25 |
| Cultivar | family | −117.1 | 0.0 | 0.82 | −117.0 | 0.0 | 0.81 |
| Cultivar | fixed only | −114.1 | 3.0 | 0.18 | −114.0 | 2.9 | 0.19 |

(C) Number of leaves

| Subset | Model | AIC | ΔAIC | *w*AIC | AICc | ΔAICc | *w*AICc |
| :-- | :-- | --: | --: | --: | --: | --: | --: |
| Total | family | 742.0 | 0.0 | 0.61 | 742.1 | 0.0 | 0.61 |
| Total | origin + family | 744.0 | 2.0 | 0.22 | 744.2 | 2.1 | 0.22 |
| Total | fixed only | 744.6 | 2.6 | 0.17 | 744.7 | 2.5 | 0.17 |
| AP | family | 605.5 | 0.0 | 0.64 | 605.6 | 0.0 | 0.65 |
| AP | AP + family | 606.7 | 1.2 | 0.36 | 606.9 | 1.3 | 0.35 |
| Native | fixed only | 198.9 | 0.0 | 0.73 | 199.2 | 0.0 | 0.75 |
| Native | family | 200.9 | 2.0 | 0.27 | 201.4 | 2.2 | 0.25 |
| Cultivar | family | 545.8 | 0.0 | 0.87 | 546.0 | 0.0 | 0.87 |
| Cultivar | fixed only | 549.7 | 3.9 | 0.13 | 549.8 | 3.8 | 0.13 |

(D) Above-ground biomass

| Subset | Model | AIC | ΔAIC | *w*AIC | AICc | ΔAICc | *w*AICc |
| :-- | :-- | --: | --: | --: | --: | --: | --: |
| Total | origin + family | 908.4 | 0.0 | 0.68 | 908.6 | 0.0 | 0.68 |
| Total | family | 910.0 | 1.5 | 0.32 | 910.1 | 1.5 | 0.32 |
| Total | fixed only | 931.5 | 23.1 | 0.00 | 931.6 | 23.0 | 0.00 |
| AP | family | 738.5 | 0.0 | 0.73 | 738.7 | 0.0 | 0.74 |
| AP | AP + family | 740.5 | 2.0 | 0.27 | 740.7 | 2.0 | 0.26 |
| Native | fixed only | 273.6 | 0.0 | 0.73 | 273.9 | 0.0 | 0.75 |
| Native | family | 275.6 | 2.0 | 0.27 | 276.1 | 2.2 | 0.25 |
| Cultivar | family | 627.1 | 0.0 | 0.98 | 627.3 | 0.0 | 0.98 |
| Cultivar | fixed only | 634.5 | 7.4 | 0.02 | 634.6 | 7.4 | 0.02 |

(E) Below-ground biomass

| Subset | Model | AIC | ΔAIC | *w*AIC | AICc | ΔAICc | *w*AICc |
| :-- | :-- | --: | --: | --: | --: | --: | --: |
| Total | family | 1067.1 | 0.0 | 0.68 | 1067.2 | 0.0 | 0.69 |
| Total | origin + family | 1068.7 | 1.6 | 0.31 | 1068.8 | 1.6 | 0.31 |
| Total | fixed only | 1077.4 | 10.3 | 0.00 | 1077.4 | 10.2 | 0.00 |
| AP | family | 857.4 | 0.0 | 0.73 | 857.6 | 0.0 | 0.74 |
| AP | AP + family | 859.4 | 2.0 | 0.27 | 859.6 | 2.1 | 0.26 |
| Native | fixed only | 335.5 | 0.0 | 0.73 | 335.8 | 0.0 | 0.75 |
| Native | family | 337.5 | 2.0 | 0.27 | 338.0 | 2.2 | 0.25 |
| Cultivar | family | 721.3 | 0.0 | 0.83 | 721.5 | 0.0 | 0.82 |
| Cultivar | fixed only | 724.5 | 3.2 | 0.17 | 724.6 | 3.1 | 0.18 |

(F) Leaf biomass

| Subset | Model | AIC | ΔAIC | *w*AIC | AICc | ΔAICc | *w*AICc |
| :-- | :-- | --: | --: | --: | --: | --: | --: |
| Total | origin + family | 1239.5 | 0.0 | 0.92 | 1239.7 | 0.0 | 0.92 |
| Total | family | 1244.5 | 4.9 | 0.08 | 1244.6 | 4.9 | 0.08 |
| Total | fixed only | 1250.5 | 11.0 | 0.00 | 1250.6 | 10.9 | 0.00 |
| AP | family | 1016.5 | 0.0 | 0.66 | 1016.6 | 0.0 | 0.66 |
| AP | AP + family | 1017.8 | 1.3 | 0.34 | 1018.0 | 1.4 | 0.34 |
| Native | fixed only | 368.7 | 0.0 | 0.73 | 369.0 | 0.0 | 0.75 |
| Native | family | 370.7 | 2.0 | 0.27 | 371.2 | 2.2 | 0.25 |
| Cultivar | fixed only | 864.7 | 0.0 | 0.57 | 864.8 | 0.0 | 0.58 |
| Cultivar | family | 865.2 | 0.6 | 0.43 | 865.4 | 0.6 | 0.42 |

(G) Stem biomass

| Subset | Model | AIC | ΔAIC | *w*AIC | AICc | ΔAICc | *w*AICc |
| :-- | :-- | --: | --: | --: | --: | --: | --: |
| Total | family | 826.2 | 0.0 | 0.60 | 826.3 | 0.0 | 0.60 |
| Total | origin + family | 826.9 | 0.8 | 0.40 | 827.1 | 0.8 | 0.40 |
| Total | fixed only | 856.9 | 30.7 | 0.00 | 857.0 | 30.7 | 0.00 |
| AP | family | 660.6 | 0.0 | 0.73 | 660.8 | 0.0 | 0.74 |
| AP | AP + family | 662.6 | 2.0 | 0.27 | 662.8 | 2.0 | 0.26 |
| Native | fixed only | 253.8 | 0.0 | 0.73 | 254.1 | 0.0 | 0.75 |
| Native | family | 255.8 | 2.0 | 0.27 | 256.3 | 2.2 | 0.25 |
| Cultivar | family | 566.6 | 0.0 | 1.00 | 566.8 | 0.0 | 1.00 |
| Cultivar | fixed only | 580.7 | 14.1 | 0.00 | 580.8 | 14.0 | 0.00 |
...
| :-- | :-- | :-- | :-- | :-- | :-- | :-- | :-- |
| Plant height | origin + family | original | — | 0.023 [0, 0.127]** | 0.005 [0.001, 0.011]*** | 0.050 [0.043, 0.057] | 0.276 [0, 0.641] |
| Plant height | origin + family | obs | 2 | 0.021 [0, 0.109] | 0.006 [0.002, 0.011] | 0.047 [0.040, 0.054] | 0.311 [0.061, 0.676] |
| Plant height | origin + family | fam | urb09 | 0.012 [0, 0.062] | 0.002 [0, 0.006] | 0.050 [0.043, 0.058] | 0.142 [0, 0.380] |
...
| Stem diameter | origin + family | original | — | 0.003 [0, 0.020] | 0.002 [0, 0.005]* | 0.043 [0.037, 0.049] | 0.163 [0, 0.396] |
| Stem diameter | origin + family | obs | 4 | 0.003 [0, 0.015] | 0.002 [0, 0.005] | 0.038 [0.032, 0.043] | 0.195 [0, 0.425] |
| Stem diameter | origin + family | fam | urb09 | 0.000 [0, 0.006] | 0.001 [0, 0.003] | 0.044 [0.038, 0.050] | 0.079 [0, 0.290] |
| Stem diameter | family | original | — | — | 0.003 [0, 0.006]** | 0.043 [0.037, 0.048] | 0.226 [0, 0.504] |
| Stem diameter | family | obs | 4 | — | 0.003 [0, 0.005] | 0.038 [0.033, 0.043] | 0.251 [0.021, 0.523] |
| Stem diameter | family | fam | urb03 | — | 0.002 [0, 0.005] | 0.043 [0.037, 0.050] | 0.193 [0, 0.443] |
| Number of leaves | origin + family | original | — | 0.000 [0, 0.024] | 0.012 [0, 0.030]* | 0.275 [0.240, 0.314] | 0.168 [0, 0.412] |
| Number of leaves | origin + family | obs | 5 | 0.000 [0, 0.029] | 0.016 [0.001, 0.034] | 0.234 [0.202, 0.265] | 0.250 [0.010, 0.509] |
| Number of leaves | origin + family | fam | urb09 | 0.000 [0, 0.023] | 0.004 [0, 0.019] | 0.280 [0.242, 0.319] | 0.052 [0, 0.266] |
| Number of leaves | family | original | — | — | 0.012 [0, 0.031]* | 0.275 [0.237, 0.316] | 0.168 [0, 0.431] |
| Number of leaves | family | obs | 5 | — | 0.016 [0.001, 0.034] | 0.234 [0.201, 0.268] | 0.250 [0.015, 0.550] |
| Number of leaves | family | fam | urb09 | — | 0.004 [0, 0.017] | 0.280 [0.242, 0.320] | 0.052 [0, 0.248] |
| Above-ground biomass | origin + family | original | — | 0.127 [0, 0.666]** | 0.022 [0, 0.048]** | 0.380 [0.330, 0.433] | 0.165 [0, 0.436] |
| Above-ground biomass | origin + family | obs | 6 | 0.119 [0, 0.627] | 0.024 [0.003, 0.053] | 0.313 [0.273, 0.356] | 0.207 [0.025, 0.527] |
| Above-ground biomass | origin + family | fam | urb09 | 0.058 [0, 0.294] | 0.004 [0, 0.025] | 0.384 [0.333, 0.436] | 0.039 [0, 0.235] |
...
| Above-ground biomass | family | fam | urb09 | — | 0.013 [0, 0.038] | 0.384 [0.330, 0.438] | 0.130 [0, 0.380] |
| Below-ground biomass | origin + family | original | — | 0.089 [0, 0.482]· | 0.029 [0, 0.065]* | 0.541 [0.467, 0.618] | 0.175 [0, 0.421] |
| Below-ground biomass | origin + family | obs | 7 | 0.076 [0, 0.393] | 0.047 [0.011, 0.091] | 0.425 [0.367, 0.485] | 0.341 [0.083, 0.673] |
| Below-ground biomass | origin + family | fam | urb09 | 0.025 [0, 0.147] | 0.007 [0, 0.032] | 0.546 [0.468, 0.618] | 0.046 [0, 0.223] |
...
| Leaf biomass | origin + family | original | — | 0.241 [0, 1.295]** | 0.010 [0, 0.046] | 0.794 [0.690, 0.905] | 0.039 [0, 0.211] |
| Leaf biomass | origin + family | obs | 8 | 0.160 [0, 0.854] | 0.017 [0, 0.050] | 0.537 [0.463, 0.612] | 0.093 [0, 0.316] |
| Leaf biomass | origin + family | fam | urb09 | 0.131 [0, 0.667] | 0.000 [0, 0.034] | 0.812 [0.702, 0.920] | 0.000 [0, 0.154] |
| Leaf biomass | family | original | — | — | 0.045 [0, 0.104]** | 0.796 [0.691, 0.909] | 0.215 [0, 0.469] |
| Leaf biomass | family | obs | 7 | — | 0.044 [0.007, 0.096] | 0.551 [0.475, 0.631] | 0.295 [0.051, 0.610] |
| Leaf biomass | family | fam | urb13 | — | 0.043 [0, 0.100] | 0.741 [0.646, 0.848] | 0.218 [0, 0.505] |
| Stem biomass | origin + family | original | — | 0.093 [0, 0.461]* | 0.031 [0.007, 0.064]*** | 0.316 [0.275, 0.362] | 0.282 [0.066, 0.641] |
| Stem biomass | origin + family | obs | 5 | 0.087 [0, 0.455] | 0.032 [0.008, 0.061] | 0.273 [0.237, 0.311] | 0.328 [0.076, 0.689] |
| Stem biomass | origin + family | fam | urb09 | 0.033 [0, 0.179] | 0.010 [0, 0.032] | 0.319 [0.276, 0.364] | 0.113 [0, 0.359] |
...
| Stem biomass | family | fam | urb09 | — | 0.016 [0, 0.038] | 0.318 [0.276, 0.368] | 0.188 [0, 0.441] |
...
| :-- | :-- | :-- | :-- | :-- | :-- | :-- |
| Plant height | Native | original | — | 0.003 [0, 0.014] | 0.055 [0.042, 0.069] | 0.238 [0, 0.877] |
| Plant height | Native | obs | 1 | 0.004 [0, 0.013] | 0.051 [0.040, 0.064] | 0.275 [0, 0.832] |
| Plant height | Native | fam | nat19 | 0.005 [0, 0.017] | 0.049 [0.037, 0.064] | 0.356 [0, 1.053] |
...
| Plant height | Cultivar | fam | urb09 | 0.002 [0, 0.006] | 0.047 [0.039, 0.055] | 0.163 [0, 0.458] |
| Stem diameter | Native | original | — | 0.000 [0, 0.006] | 0.052 [0.039, 0.064] | 0.008 [0, 0.457] |
| Stem diameter | Native | obs | 1 | 0.000 [0, 0.007] | 0.045 [0.035, 0.056] | 0.040 [0, 0.539] |
| Stem diameter | Native | fam | nat19 | 0.000 [0, 0.007] | 0.047 [0.035, 0.060] | 0.040 [0, 0.542] |
| Stem diameter | Cultivar | original | — | 0.003 [0, 0.006]* | 0.038 [0.032, 0.044] | 0.247 [0, 0.592] |
| Stem diameter | Cultivar | obs | 3 | 0.002 [0, 0.005] | 0.035 [0.029, 0.040] | 0.193 [0, 0.502] |
| Stem diameter | Cultivar | fam | urb11 | 0.002 [0, 0.005] | 0.037 [0.031, 0.043] | 0.181 [0, 0.493] |
| Number of leaves | Native | original | — | 0.000 [0, 0.000] | 0.241 [0.188, 0.300] | 0.000 [0, 0.000] |
| Number of leaves | Native | obs | 1 | 0.000 [0, 0.000] | 0.223 [0.172, 0.277] | 0.000 [0, 0.000] |
| Number of leaves | Native | fam | nat19 | 0.000 [0, 0.000] | 0.258 [0.197, 0.329] | 0.000 [0, 0.000] |
| Number of leaves | Cultivar | original | — | 0.020 [0, 0.051]** | 0.289 [0.245, 0.336] | 0.263 [0, 0.622] |
| Number of leaves | Cultivar | obs | 4 | 0.025 [0.003, 0.054] | 0.239 [0.201, 0.276] | 0.378 [0.049, 0.779] |
| Number of leaves | Cultivar | fam | urb09 | 0.009 [0, 0.032] | 0.296 [0.246, 0.342] | 0.123 [0, 0.405] |
| Above-ground biomass | Native | original | — | 0.000 [0, 0.039] | 0.410 [0.307, 0.506] | 0.000 [0, 0.365] |
| Above-ground biomass | Native | obs | 2 | 0.000 [0, 0.000] | 0.325 [0.252, 0.403] | 0.000 [0, 0.000] |
| Above-ground biomass | Native | fam | nat29 | 0.000 [0, 0.062] | 0.403 [0.304, 0.491] | 0.000 [0, 0.575] |
...
| Above-ground biomass | Cultivar | fam | urb09 | 0.009 [0, 0.032] | 0.369 [0.310, 0.428] | 0.090 [0, 0.327] |
| Below-ground biomass | Native | original | — | 0.017 [0, 0.113] | 0.630 [0.468, 0.778] | 0.103 [0, 0.666] |
| Below-ground biomass | Native | obs | 1 | 0.023 [0, 0.109] | 0.529 [0.409, 0.657] | 0.165 [0, 0.722] |
| Below-ground biomass | Native | fam | nat19 | 0.028 [0, 0.117] | 0.512 [0.378, 0.657] | 0.209 [0, 0.821] |
| Below-ground biomass | Cultivar | original | — | 0.031 [0, 0.082]* | 0.496 [0.420, 0.574] | 0.233 [0, 0.589] |
| Below-ground biomass | Cultivar | obs | 5 | 0.040 [0.004, 0.089] | 0.398 [0.336, 0.459] | 0.363 [0.038, 0.776] |
| Below-ground biomass | Cultivar | fam | urb09 | 0.005 [0, 0.038] | 0.494 [0.409, 0.571] | 0.042 [0, 0.291] |
| Leaf biomass | Native | original | — | 0.000 [0, 0.095] | 0.817 [0.605, 1.003] | 0.000 [0, 0.440] |
| Leaf biomass | Native | obs | 2 | 0.000 [0, 0.083] | 0.647 [0.490, 0.797] | 0.000 [0, 0.494] |
| Leaf biomass | Native | fam | nat19 | 0.000 [0, 0.106] | 0.789 [0.578, 1.001] | 0.000 [0, 0.493] |
| Leaf biomass | Cultivar | original | — | 0.026 [0, 0.091] | 0.775 [0.655, 0.898] | 0.130 [0, 0.427] |
| Leaf biomass | Cultivar | obs | 6 | 0.031 [0, 0.079] | 0.484 [0.406, 0.560] | 0.238 [0, 0.585] |
| Leaf biomass | Cultivar | fam | urb09 | 0.002 [0, 0.042] | 0.809 [0.678, 0.936] | 0.010 [0, 0.204] |
| Stem biomass | Native | original | — | 0.008 [0, 0.060] | 0.349 [0.259, 0.431] | 0.090 [0, 0.635] |
| Stem biomass | Native | obs | 2 | 0.019 [0, 0.077] | 0.279 [0.213, 0.349] | 0.252 [0, 0.936] |
| Stem biomass | Native | fam | nat19 | 0.019 [0, 0.076] | 0.311 [0.229, 0.399] | 0.235 [0, 0.861] |
...
| Stem biomass | Cultivar | fam | urb09 | 0.011 [0, 0.035] | 0.303 [0.254, 0.355] | 0.141 [0, 0.431] |
...
| :-- | --: | --: | --: | --: | --: | --: |
| plant + family | −915.2 | 0.0 | 0.52 | −915.2 | 0.0 | 0.52 |
| plant + family + origin | −915.1 | 0.1 | 0.48 | −915.0 | 0.2 | 0.48 |
| plant + origin | −859.0 | 56.3 | 0.00 | −858.9 | 56.3 | 0.00 |
| plant | −801.4 | 113.9 | 0.00 | −801.3 | 113.9 | 0.00 |
| fixed only | −96.9 | 818.4 | 0.00 | −96.8 | 818.3 | 0.00 |

(B) Stem diameter

| Model | AIC | ΔAIC | *w*AIC | AICc | ΔAICc | *w*AICc |
| :-- | --: | --: | --: | --: | --: | --: |
| plant + family | −1389.8 | 0.0 | 0.73 | −1389.7 | 0.0 | 0.73 |
| plant + family + origin | −1387.8 | 2.0 | 0.27 | −1387.7 | 2.0 | 0.27 |
| plant | −1346.9 | 42.9 | 0.00 | −1346.8 | 42.9 | 0.00 |
| plant + origin | −1346.4 | 43.4 | 0.00 | −1346.3 | 43.4 | 0.00 |
| fixed only | −1062.4 | 327.4 | 0.00 | −1062.4 | 327.3 | 0.00 |

(C) Number of leaves

| Model | AIC | ΔAIC | *w*AIC | AICc | ΔAICc | *w*AICc |
| :-- | --: | --: | --: | --: | --: | --: |
| plant + family | 2321.2 | 0.0 | 0.73 | 2321.3 | 0.0 | 0.73 |
| plant + family + origin | 2323.2 | 2.0 | 0.27 | 2323.3 | 2.0 | 0.27 |
| plant + origin | 2342.1 | 20.9 | 0.00 | 2342.2 | 20.9 | 0.00 |
| plant | 2342.1 | 21.0 | 0.00 | 2342.2 | 20.9 | 0.00 |
| fixed only | 2469.8 | 148.7 | 0.00 | 2469.9 | 148.6 | 0.00 |
...
| :-- | :-- | :-- | :-- | :-- | :-- | :-- |
| Plant height | origin + family | original | 0.021 [0, 0.111]* | 0.008 [0.004, 0.014]*** | 0.023 [0.019, 0.027]*** | 0.438 [0.150, 0.849] |
| Plant height | origin + family | obs | 0.019 [0, 0.098] | 0.009 [0.004, 0.014] | 0.020 [0.017, 0.024] | 0.517 [0.185, 0.957] |
| Plant height | origin + family | fam | 0.009 [0, 0.052] | 0.006 [0.002, 0.011] | 0.023 [0.019, 0.026] | 0.381 [0.137, 0.691] |
...
| Stem diameter | origin + family | original | 0.000 [0, 0.006] | 0.003 [0.001, 0.005]*** | 0.010 [0.008, 0.012]*** | 0.291 [0.109, 0.496] |
| Stem diameter | origin + family | obs | 0.001 [0, 0.009] | 0.003 [0.001, 0.004] | 0.009 [0.007, 0.010] | 0.337 [0.143, 0.556] |
| Stem diameter | origin + family | fam | 0.002 [0, 0.013] | 0.002 [0.001, 0.004] | 0.010 [0.008, 0.012] | 0.250 [0.072, 0.452] |
...
| Number of leaves | origin + family | original | 0.003 [0, 0.030] | 0.008 [0.001, 0.016]*** | 0.043 [0.033, 0.055]*** | 0.153 [0.026, 0.296] |
| Number of leaves | origin + family | obs | 0.004 [0, 0.032] | 0.008 [0.002, 0.015] | 0.043 [0.033, 0.054] | 0.167 [0.044, 0.311] |
| Number of leaves | origin + family | fam | 0.008 [0, 0.053] | 0.007 [0.002, 0.015] | 0.041 [0.030, 0.053] | 0.134 [0.031, 0.263] |
...
| Number of leaves | family | fam | — | 0.003 [0, 0.008] | 0.043 [0.032, 0.054] | 0.060 [0, 0.152] |
...
| :-- | :-- | :-- |
| AIC | family | AICc = 4636.5, ΔAICc = 0.00, *w*AICc = 0.73 |
| AIC | origin + family | AICc = 4638.5, ΔAICc = 2.04, *w*AICc = 0.27 |
| AIC | fixed only | AICc = 4662.6, ΔAICc = 26.1, *w*AICc = 0.00 |
| Variance | *V*origin | 10.7 [0, 71.8] |
| Variance | *V*family | 23.3 [7.6, 42.2] |
...
![](media/fig_survival.png)

**Fig_Survival.** Proportion of *Cornus florida* seedlings alive at each census in the common garden, for native-origin (blue) and cultivar-origin (orange) seedlings, classified by maternal-tree location. Points are observed proportions; shaded bands are 95% confidence intervals. Survival to the final census was 79.3% for native-origin and 83.5% for cultivar-origin seedlings.
